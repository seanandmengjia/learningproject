...
public class PeerHandle {
	...
	public PeerHandle(Config config, int portToBind) {
		...
		final ConnectionService connectionService = new ConnectionService(config, networkEventLoopGroup,
				peerEventLoopGroup, encoder);
		final LeadershipService leadershipService = new LeadershipService(connectionService, config,
				peerEventLoopGroup);
	...
	public ChannelFuture start() throws InterruptedException {
		...
		if (bindFuture.isSuccess()) {
			...
			this.keepAliveFuture = peerEventLoopGroup.scheduleAtFixedRate((Runnable) peer::keepAlivePing, initialDelay,
					config.getKeepAlivePeriodSeconds(), SECONDS);

			this.timeoutPingsFuture = peerEventLoopGroup.scheduleAtFixedRate((Runnable) peer::timeoutPings, 0, 100,
	...
	public CompletableFuture<Collection<Void>> send(String peerName, String msg) {
		final CompletableFuture<Collection<Void>> future = new CompletableFuture<>();
		peerEventLoopGroup.execute(() -> peer.send(peerName, msg, future));
		return future;
	}
