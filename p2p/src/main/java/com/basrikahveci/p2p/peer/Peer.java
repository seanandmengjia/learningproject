...
import com.basrikahveci.p2p.peer.network.message.ContentMessage;
...
public class Peer {
	...
	public Peer(Config config, ConnectionService connectionService, PingService pingService,
	...
	public void handleConnectionOpened(Connection connection, String leaderName) {
		...
		if (leaderName != null) {
			...
			if (currentLeaderName == null) {
				...
				LOGGER.info("Known leader {} and leader {} announced by {} are different.", currentLeaderName,
	...
	public void timeoutPings() {
		...
		final int availableConnectionSlots = config.getMinNumberOfActiveConnections()
				- connectionService.getNumberOfConnections();

		if (availableConnectionSlots > 0) {
			List<Pong> notConnectedPeers = new ArrayList<>();
			for (Pong pong : pongs) {
				if (!config.getPeerName().equals(pong.getPeerName())
						&& !connectionService.isConnectedTo(pong.getPeerName())) {
			...
			for (int i = 0, j = min(availableConnectionSlots, notConnectedPeers.size()); i < j; i++) {
				...
				LOGGER.info("Auto-connecting to {} via {}:{}", peerToConnect.getPeerName(), peerToConnect.getPeerName(),
						host, port);
	...
	public void handleLeader(final Connection connection, String leaderName) {
		if (isShutdown()) {
			LOGGER.warn("Leader announcement of {} from connection {} ignored since not running", leaderName,
	...
	public void send(final String peerName, final String message,final CompletableFuture<Collection<Void>> futureToNotify) {
		...
		LOGGER.info("send {} to {}", message, peerName);
		Connection c = connectionService.getConnection(peerName);
		final ContentMessage oContentMessage = new ContentMessage(message);
		c.send(oContentMessage);
	}
